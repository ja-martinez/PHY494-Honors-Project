...
def compare_simpleDouble():
    ...
    """
    tSimple, phiSimple = pd.simplePendulum(0.5, 0.5, 0.5, 0, .0001, 75)
    tDouble, phiDouble = pd.rk4DobPend(0.5, 0.0000001, 0.5, 0.0000001, 0.5, 0.0, 0.0, 0.0, .0001, 75, linearized=False, \
                                       purpose="comparison")
...
def compare_linearNonlinear():
    ...
    """
    tSmall, phiSmall = pd.rk4DobPend(0.1, 0.1, 0.5, 0.5, 0.01, 0.0, 0.0, 0.0, .0001, 75, linearized=True, purpose="comparison")
    tGen, phiGen = pd.rk4DobPend(0.1, 0.1, 0.5, 0.5, 0.01, 0.0, 0.0, 0.0, .0001, 75, linearized=False, purpose="comparison")
...
def graphEnergy(L1, L2, m1, m2, phi1, phi2, w1, w2, t, g=9.81, filename='Energy'):
    ...
    ax2.plot(t, H, label=r"Hamiltonian")
...
def bifurcationMass(tmax=50, tsteps=0.01, massMax=10, massMin=1, massSteps=0.1, filename="bifurcationMass"):
    ...
    plt.plot(massPointsArray[::2], np.abs(Mass[::2, 4900::]),'r.', color='black', ms=1, alpha=.1);
...
def allGraphs(m1, m2, L1, L2, phi10, phi20, w10, w20, h, t_max, run):
    data = pd.rk4DobPend(m1, m2, L1, L2, phi10, phi20, w10, w20, h, t_max, linearized=False)
    t = data[0]
    phi1 = data[1]
    phi2 = data[2]
    w1 = data[3]
    ...
    graphTrajectory(L1, L2, phi1, phi2, filename=(run+" trajectory"))
    graphEnergy(L1, L2, m1, m2, phi1, phi2, w1, w2, t, filename=(run+"energy"))
    graphPhaseDiagrams(phi1, phi2, w1, w2, filename=(run+" Phase Diagram"))
